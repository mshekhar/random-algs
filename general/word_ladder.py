...
class Solution(object):
    ...
    def get_valid_neighbours(self, word, wordlist_map):
        s = time.time()
        for ch in string.ascii_lowercase:
            for c, i in enumerate(word):
                if i != ch:
                    new_word = word[:c] + ch + word[c + 1:]
                    if new_word in wordlist_map:
                        if new_word not in self.yield_freq:
                            self.yield_freq[new_word] = 0
                        self.yield_freq[new_word] += 1
                        yield new_word
        self.valid_n_time += time.time() - s
    ...
    def findLadders(self, beginWord, endWord, wordList):
        ...
        """
        st = time.time()
        wordlist_map = {w: float('inf') for w in wordList}
        parents_map = {w: set() for w in wordList}
        if not wordlist_map or endWord not in wordlist_map:
            return []
        q = Queue()
        q.put((beginWord, 0), )
        ...
        while q.qsize() > 0:
            word, path_len = q.get()
            # print 'processing ', word, path_len
            wordlist_map[word] = path_len
            for nei in self.get_valid_neighbours(word, wordlist_map):
                if nei == endWord:
                    # print word, path_len, nei, min_path_len
                    if path_len + 1 <= min_path_len:
                        parents_map[nei].add(word)
                        min_path_len = path_len + 1
                else:
                    if wordlist_map[nei] >= path_len+1:
                        if wordlist_map[nei] > path_len + 1:
                            wordlist_map[nei] = path_len + 1
                            q.put((nei, path_len + 1), )
                        parents_map[nei].add(word)

        # print parents_map
        # print wordlist_map
        # print min_path_len
        # print 'bfs', time.time() - st
        st = time.time()

        res = []
        if min_path_len != float('inf'):
            paths_queue = Queue()
            for parent in parents_map[endWord]:
                paths_queue.put((parent, [endWord]))
            while paths_queue.qsize() > 0:
                parent, path = paths_queue.get()
                copy_path = path[:]
                copy_path.insert(0, parent)
                if parent != beginWord:
                    for p in parents_map[parent]:
                        paths_queue.put((p, copy_path))
                else:
                    res.append(copy_path)
        # print 'res ctr', time.time() - st
        # print 'valid n time', self.valid_n_time
        # print sorted(self.yield_freq.items(), key=lambda x:x[1], reverse=True)
        return res
...
print Solution().findLadders("nanny", "aloud",
